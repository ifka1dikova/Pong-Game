/* The PONG game
The Pong Game was designed by Allan Alcorn, manufactured by Atari and released in 1972.
 The game is a two-dimensional sports game that simulates table tennis. 
 Each player controls a paddle by moving it vertically across the left or right 
 side of the screen. Both players use the paddles to hit the ball back and forth.
 The goal is for each player to reach 10 points before the opponent. 
 The player wins a point when his opponent fails to return the ball to him. 
 Player one controls are UP and Down ARROW. Player two controls are W and S keys! 
 Each time when a player reaches 10 points both players go to the next level with 
 a slightly different design. 

*/


var level =1    // starting the game from level one 


var BALL_SPEED =5;  // global veriable for the ball speed
...
function setup() {
  ...
  song.loop();         // repeating the background song
 ...
 function draw() {
     if (level ==1){          // if statement instructing the game to start from level one 
         background(55,149,171);  // background colour level one (ping pong table colour-blueish)
	line(width/2 ,1,width/2,3000);  //  white line in the middle of the screen separating both fields 
    stroke(255);
         text("Controls:Player1 W and S,Player2 UP and DOWN", width/2, 20);  // text for the controls appears only in level 1 

         
     }
     else if (level ==2){   // else if statement  for starting level two 
    ...
    stroke(255);
      ...
      else if (level ==3){   // else if statement  for starting level three
  background(219,206,55);    // background colour level three is yellow (beach badminton court)
	line(width/2 ,1,width/2,3000);  // white line in the middle of the screen separating both fields
...
window.addEventListener("keydown", arrow_keys_handler, false);
 ...
 function checklevel(){     // function for cheking the level of the game
     if ((paddle1Score ==10) || (paddle2Score==10)){   // if one of the players make 10 points go to next level
         
         if(level<3){    // if goes to level 3 continue playing on this level
            level++ ;
         }
         
         reset ();
         paddle1Score =0    // both players start with score 0
         paddle2Score =0
...
/* LEVEL 1 
   appearance : ping pong table 

    LEVEL 2 
   appearance : tennis court 
 
   LEVEL 3 
   appearance : badminton court 
	
 
 References :
 [1] Youtube video for the Pong Game 
 https://www.youtube.com/watch?v=DhRJb58tOlA
 [2] Keyboard Keys and Key Code Values
